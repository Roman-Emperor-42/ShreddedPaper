...
public class SynchronousPluginExecution {
    ...
    public static void execute(Plugin plugin, RunnableWithException runnable) throws Exception {
        ...
        if (plugin == null || config == null || !config.multithreading.runUnsupportedPluginsInSync || plugin.getPluginMeta().isFoliaSupported() || TickThread.isShutdownThread()) {            // Multi-thread safe plugin, run it straight away
            runnable.run();
            return;
        }

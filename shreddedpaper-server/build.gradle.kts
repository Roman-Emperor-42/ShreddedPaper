...
paperweight {
    ...
    // ShreddedPaper start - Rebrand
    val shreddedpaper = forks.register("shreddedpaper") {
            forks = purpur
            upstream.patchRepo("paperServer") {
                    upstreamRepo = purpur.patchedRepo("paperServer")
                    patchesDir = rootDirectory.dir("shreddedpaper-server/paper-patches")
                    outputDir = rootDirectory.dir("paper-server")
                }
            upstream.patchDir("purpurServer") {
                    ...
                    patchesDir = rootDirectory.dir("shreddedpaper-server/purpur-patches")
                    outputDir = rootDirectory.dir("purpur-server")
                }
        }
    activeFork = shreddedpaper
    // ShreddedPaper end - Rebrand
...
dependencies {
    implementation(project(":shreddedpaper-api")) // Purpur
...
tasks.jar {
    manifest {
        ...
        attributes(
            ...
            "Implementation-Title" to "ShreddedPaper", // Purpur
            "Implementation-Version" to implementationVersion,
            "Implementation-Vendor" to date,
            "Specification-Title" to "ShreddedPaper", // Purpur
            "Specification-Version" to project.version,
            "Specification-Vendor" to "ShreddedPaper Team", // Purpur
            "Brand-Id" to "multipaper:shreddedpaper", // Purpur
            "Brand-Name" to "ShreddedPaper", // Purpur
...
val preparePaperServerGit = tasks.register<Exec>("preparePaperServerGit") {
    workingDir = layout.projectDirectory.dir("../paper-server").asFile
    isIgnoreExitValue = true
    commandLine(
        "bash", "-lc",
        """
        git am --abort || git am --quit || true
        git rebase --abort || true
        git switch -c patched || git switch -C patched
        """.trimIndent()
    )
}

val preparePurpurServerGit = tasks.register<Exec>("preparePurpurServerGit") {
    workingDir = layout.projectDirectory.dir("../purpur-server").asFile
    isIgnoreExitValue = true
    commandLine(
        "bash", "-lc",
        """
...
tasks.withType<RebuildFilePatches>().configureEach {
    if (name.contains("PaperServer")) {
        dependsOn(preparePaperServerGit)
    }
    if (name.contains("PurpurServer")) {
        dependsOn(preparePurpurServerGit)
    }
}
